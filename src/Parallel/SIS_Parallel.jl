...
#println(parallel);
    ...
    for iter=1:niter
        ...
        #Conversion
        henextstatus =convert(SharedArray,henextstatus)
        hestatus=convert(SharedArray,hestatus)
        vnextstatus=convert(SharedArray,vnextstatus)
 ...
 lk_7=ReentrantLock()
      ...
      for t=1:length(intervals)
            ...
            #
            Threads.@threads for he=1:nhe(h)
            ...
            _vstatus= convert(SharedArray,_vstatus)

            #
            # PHASE 2 - Agent-to-Agent
            #
            avg_direct_contacts = 0
           Threads.@threads for v=1:nhv(h)
